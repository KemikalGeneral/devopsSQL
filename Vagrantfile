...
Vagrant.configure("2") do |config|
  ...
  # documentation for more information about their specific syntax and use.
   config.vm.provision "shell", inline: <<-SHELL
    ...
    echo -e "\nCloning the Northwind repository...\n"
    git clone https://gitlab.cs.cf.ac.uk/CM6212/northwind.git
    #Change to the northwind directory
    cd northwind
    #Enter username & password for mysql, and load the Northwind files
    sudo mysql --user=root --password=password < northwind.sql
    sudo mysql --user=root --password=password < northwind-data.sql
    #Clone the devopsSQL repo
    echo -e "\nCloning the Student repository...\n"
    cd ..
    git clone https://github.com/KemikalGeneral/devopsSQL.git
    #Change to the devopsSQL directory
    cd devopsSQL
